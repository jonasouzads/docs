...
| 404 | `MESSAGE_NOT_FOUND` | A mensagem não existe, ou ainda não foi gravada | Ver [Status da mensagem](/api-reference/whatsapp/status-da-mensagem) |
...
Ela aparece depois, no campo `failed_reason` da consulta de status, já traduzida para português. Veja [Status da mensagem](/api-reference/whatsapp/status-da-mensagem).
