...
<Warning>
  O encaminhamento cobre **mensagens recebidas** (`message.received`). Ele **não** notifica mudança de status de mensagens que você enviou. Para saber se um envio foi entregue ou lido, use a [consulta de status](/api-reference/whatsapp/status-da-mensagem).
</Warning>
