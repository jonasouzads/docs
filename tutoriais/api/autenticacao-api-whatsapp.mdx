...
<Warning>
  **Se você seguiu uma versão anterior desta documentação, sua integração não funciona.** Aquela versão instruía a enviar a chave como parâmetro `apiToken` no corpo da requisição. Esse parâmetro **nunca foi aceito pela API** — requisições nesse formato sempre receberam `401`. Migre para o cabeçalho abaixo.
</Warning>
